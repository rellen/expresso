...
- `docs/overlays.md` — the design for overlays, which are the steps inside one slide. The
  code contains `Expresso.Overlay` and the entities of the DSL, and not the transformer,
  the CSS or the presenter. The document ends with five decisions, and each is settled.
